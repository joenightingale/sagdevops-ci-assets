...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="buildISHost"/>
	<xsl:param name="buildISPort"/>
	<xsl:param name="buildISUsername"/>
	<xsl:param name="buildISPassword"/>
	...
	<xsl:template match="DeployerSpec/Environment">
	    <Environment>
			<IS>
				<isalias name="buildServer">
					<host><xsl:value-of select="$buildISHost"/></host>
					<port><xsl:value-of select="$buildISPort"/></port>
					<user><xsl:value-of select="$buildISUsername"/></user>
					<pwd><xsl:value-of select="$buildISPassword"/></pwd>
					<useSSL>false</useSSL>
	...
	<xsl:template match="DeployerSpec/Projects">
		<Projects>
			...
			<xsl:attribute name="name"><xsl:value-of select="$projectName"/></xsl:attribute>			
				...
				<DeploymentMap description="" name="myDeploymentMapBuild"/>			
				<MapSetMapping mapName="myDeploymentMapBuild" setName="myDeploymentSet">								
					<alias type="IS">buildServer</alias>
				</MapSetMapping>	
				<DeploymentCandidate description="" mapName="myDeploymentMapBuild" name="myDeploymentBuild"/>
			</Project>
